...
// Utility: Get current datetime in yyyyMMddHHmmss format
...
// Create payment order and get PayPhi redirect URL
exports.createOrder = async (req, res) => {
  try {
    ...
    const txnId = "TXN" + Date.now();
    const txnDate = getCurrentDateTime();
    const returnURL = "http://13.235.80.49:3000/jioPGCallback"; // Update if needed
    ...
    console.log("📤 Sending request to PayPhi:", requestBody);
    ...
    console.log("✅ Received response from PayPhi:", responseData);

    // Save transaction details to DB
    await db.query(
      "INSERT INTO transactions (merchantTxnNo, amount, responseCode, status, created_at) VALUES (?, ?, ?, ?, NOW())",
      [txnId, payload.amount, responseData.responseCode, responseData.status || "INITIATED"]
    );
    ...
    if (responseData.responseCode === "000") {
      ...
      return res.status(500).json({
        error: "PayPhi returned an error",
